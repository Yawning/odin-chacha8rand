...
// Note: Imports kept to a minimum for eventual inclusion in base.
import "base:intrinsics"
import "base:runtime"
import "core:crypto" // For system entropy source.
...
@(require_results)
chacha8rand_random_generator :: proc "contextless" (state: ^Chacha8Rand_State = nil) -> runtime.Random_Generator {
	return {
		procedure = chacha8rand_proc,
		data = state,
	}
}

chacha8rand_proc :: proc(data: rawptr, mode: runtime.Random_Generator_Mode, p: []byte) {
	@(thread_local)
	state: Chacha8Rand_State

	r: ^Chacha8Rand_State = &state
	if data != nil {
		r = cast(^Chacha8Rand_State)data
	}
...
chacha8rand_refill :: proc(r: ^Chacha8Rand_State) {
	...
	} else when runtime.HAS_HARDWARE_SIMD && ODIN_ARCH != .i386 {
